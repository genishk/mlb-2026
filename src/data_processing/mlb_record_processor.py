...
class MLBRecordProcessor:
    """
    MLB 경기 데이터를 매치 단위 레코드로 처리하여 JSON으로 저장하는 클래스
    
    이 클래스는 MLB API에서 수집한 로데이터를 처리하여 매치 단위로 정리합니다.
    원본 데이터의 중첩 구조를 평탄화하여 단일 레벨의 JSON 구조로 변환합니다.
    """
    ...
    def __init__(self, debug_mode=True):
        ...
        # 로깅 설정
        self.logger = logging.getLogger("MLBRecordProcessor")
    ...
    def load_data(self, file_path=None, data_type='historical'):
        """데이터 파일 로드하기"""
        ...
        try:
            self.logger.info(f"{data_type.capitalize()} 데이터 로드 중: {file_path}")
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            # 데이터 구조 검증
            if not isinstance(data, dict) or 'games' not in data:
                self.logger.error(f"데이터 파일 구조가 유효하지 않습니다: 'games' 키가 없습니다")
                return None
            
            # 게임 수 로깅
            games_count = len(data['games'])
            self.logger.info(f"로드된 {data_type} 경기 수: {games_count}")
            
            # 기본 데이터 구조 확인을 위한 샘플 출력
            if self.debug_mode and games_count > 0:
                sample_game = data['games'][0]
                self.logger.debug(f"샘플 게임 구조: {list(sample_game.keys())}")
                if 'schedule' in sample_game:
                    self.logger.debug(f"schedule 구조: {list(sample_game['schedule'].keys())}")
                if 'boxscore' in sample_game:
                    self.logger.debug(f"boxscore 구조: {list(sample_game['boxscore'].keys())}")
    ...
    def process_and_save(self, data_type='historical'):
        """데이터 처리 및 저장 파이프라인"""
        # 1. 데이터 로드
        data = self.load_data(data_type=data_type)
        if not data:
            self.logger.error(f"{data_type} 데이터 로드 실패")
            return None
        
        # 2. 데이터 처리
        is_historical = (data_type == 'historical')
        records = self.process_games(data, is_historical=is_historical)
        if not records:
            self.logger.error("데이터 처리 실패")
            return None
        ...
        if records and len(records) > 0:
            ...
            # 투구/타격 통계 필드 표시
            batting_fields = [key for key in sample_record.keys() if 'batting_' in key]
            pitching_fields = [key for key in sample_record.keys() if 'pitching_' in key]
            
            if batting_fields:
                self.logger.info("\n타격 통계 (일부):")
                for field in sorted(batting_fields)[:5]:  # 일부만 표시
                    self.logger.info(f"  {field}: {sample_record[field]}")
            
            if pitching_fields:
                self.logger.info("\n투구 통계 (일부):")
                for field in sorted(pitching_fields)[:5]:  # 일부만 표시
                    self.logger.info(f"  {field}: {sample_record[field]}")
    ...
    def analyze_api_variables(self, sample_game):
        """
        API 응답의 모든 변수를 분석하고 출력
        이 메서드는 저장된 데이터의 원시 구조를 확인하는 데 유용합니다.
        """
        self.logger.info("API 변수 분석 시작...")
        
        def explore_dict(d, prefix='', max_depth=3, current_depth=0):
            """재귀적으로 딕셔너리 구조를 탐색하고 모든 키 출력"""
            if current_depth >= max_depth:
                return [f"{prefix} (더 깊은 구조 생략)"]
            ...
            if isinstance(d, dict):
                for k, v in d.items():
                    ...
                    if isinstance(v, dict):
                        variables.extend(explore_dict(v, new_prefix, max_depth, current_depth + 1))
                    elif isinstance(v, list) and v and isinstance(v[0], dict):
                        # 리스트에 딕셔너리가 있으면 첫 번째 항목만 탐색
                        variables.append(f"{new_prefix} (리스트, {len(v)}개 항목)")
                        variables.extend(explore_dict(v[0], f"{new_prefix}[0]", max_depth, current_depth + 1))
                    else:
                        variables.append(f"{new_prefix}: {type(v).__name__}")
        ...
        # 게임의 주요 구성 요소 확인
        components = ['schedule', 'boxscore', 'linescore', 'play_by_play']
        
        for component in components:
            if component in sample_game:
                self.logger.info(f"\n=== {component.upper()} 변수 ===")
                variables = explore_dict(sample_game[component], component)
                all_variables.extend(variables)
                
                # 로그에 출력 (가독성을 위해 컴포넌트별로 정렬)
                for var in sorted(variables):
                    self.logger.info(var)
        
        self.logger.info(f"\n총 {len(all_variables)}개 변수 발견")
        return all_variables

    def process_games(self, data, is_historical=True):
        """MLB 경기 데이터를 매치 단위 레코드로 변환"""
        if not data or 'games' not in data:
            self.logger.error("처리할 데이터가 없습니다.")
            return None
        
        games_count = len(data['games'])
        self.logger.info(f"경기 데이터 처리 시작 ({games_count}개 경기)")
        ...
        for i, game in enumerate(data['games']):
            try:
                ...
                game_id = game.get('id')
                if not game_id:
                    self.logger.warning("게임 ID가 없는 경기를 건너뜁니다.")
                    skipped_games += 1
                    continue
                
                # 중복 처리 방지
                if game_id in self.processed_game_ids:
                    self.logger.debug(f"게임 ID {game_id}는 이미 처리되었습니다. 건너뜁니다.")
                    skipped_games += 1
                    continue
                
                # 과거 경기이면서 완료되지 않은 경기 건너뛰기
                if is_historical and not self._is_game_completed(game):
                    self.logger.debug(f"게임 ID {game_id}는 완료되지 않은 경기입니다. 건너뜁니다.")
                    skipped_games += 1
                    continue
                ...
                # 기록 저장
                processed_records.append(game_record)
                
                # 진행상황 로깅
                if (i + 1) % 10 == 0 or i + 1 == games_count:
                    self.logger.info(f"{i + 1}/{games_count} 경기 처리 완료")
        ...
        self.logger.info(f"경기 처리 완료: {len(processed_records)}개 처리, {skipped_games}개 건너뜀")
        
        # 처리된 레코드가 있는지 확인
        if not processed_records:
            self.logger.error("처리된 경기가 없습니다.")
            return None
...
if __name__ == "__main__":
    ...
    processor = MLBRecordProcessor(debug_mode=True)
    
    # 1. API 변수 분석 (로데이터 확인)
    data = processor.load_data(data_type='historical')
    if data and data.get('games') and len(data['games']) > 0:
        processor.analyze_api_variables(data['games'][0])
    
    # 2. 과거 경기 데이터 처리
    historical_output = processor.process_and_save(data_type='historical')
    print(f"\n과거 경기 처리 결과: {historical_output}")
    
    # 3. 예정된 경기 데이터 처리
    upcoming_output = processor.process_and_save(data_type='upcoming')
