...
class MLBBettingModel1ExtendedLGBM:
    """
    MLB 경기 승패 예측을 위한 Model1 확장 버전 (LightGBM)
    
    mlb_model1과 동일한 기본 구조를 사용하되, 123개의 확장된 특성을 사용합니다.
    고급 기법 없이 단순한 LightGBM 분류기로 홈팀 승리 여부를 예측합니다.
    """
    ...
    def __init__(self):
        ...
        self.dates = None
        self.scaler = StandardScaler()  # 정의만 하고 사용하지 않음 (mlb_model1과 동일)
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        Args:
            ...
            verbose: 상세 출력 여부
        """
        print("\n=== Model1 확장 LightGBM 모델 학습 시작 ===")
        
        # 특성과 레이블 추출
        X, y = self.prepare_features(training_data)
        
        # 지수적 증가 가중치 (mlb_model1과 동일)
        n_samples = len(X)
        sample_weights = np.exp(np.linspace(0, 1, n_samples)) - 1
        sample_weights = sample_weights / sample_weights.mean()  # 정규화
        
        # 기본 LightGBM 모델 (mlb_model1과 동일한 파라미터)
        self.model = LGBMClassifier(
            n_estimators=500,              # 트리 개수
            learning_rate=0.01,            # 학습률
            max_depth=3,                   # 트리 깊이
            num_leaves=8,                  # 리프 노드 수
            min_child_samples=30,          # 최소 샘플 수
            min_child_weight=0.1,          # 최소 가중치
            subsample=0.7,                 # 데이터 샘플링 비율
            colsample_bytree=0.7,          # 특성 샘플링 비율
            reg_alpha=0.5,                 # L1 규제
    ...
    def prepare_predictions(self, game_data: List[Dict]) -> pd.DataFrame:
        ...
        if hasattr(self.model, 'n_features_'):
            model_feature_count = self.model.n_features_
        elif hasattr(self.model, 'booster_') and hasattr(self.model.booster_, 'num_feature'):
            model_feature_count = self.model.booster_.num_feature()
        else:
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"Model1 확장 LightGBM 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_model1_extended_lgbm_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_model1_extended_lgbm_features_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {
                    'type': 'lightgbm_extended',
                    'params': self.model.get_params(),
                    'feature_count': len(self.feature_names)
                }
            }, f, indent=2)
        
        print(f"\n=== Model1 확장 LightGBM 모델 저장 완료 ===")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== Model1 확장 LightGBM 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB Model1 확장 LightGBM 베팅 모델 초기화
    model = MLBBettingModel1ExtendedLGBM()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "model1_extended_lgbm")
