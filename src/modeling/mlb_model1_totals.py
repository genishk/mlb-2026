"""
MLB 총점(Over/Under) 예측 모델 1 - LightGBM Regressor
기존 mlb_model1.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModel1:
    """LightGBM 회귀 모델 - MLB 총점 예측"""
    ...
    def prepare_features(self, data: List[Dict]) -> Tuple[pd.DataFrame, pd.Series]:
        """데이터에서 특성과 타겟(총점) 추출 -- 피처는 머니라인과 동일"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """훈련 데이터를 사용하여 총점 예측 모델을 훈련"""
        print("\n=== 총점 예측 모델 학습 시작 (LightGBM Regressor) ===")
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_model1_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features1_{timestamp}.json"
    ...
    def _print_feature_importance(self) -> None:
        ...
        try:
            ...
            print("\n=== 상위 15개 중요 특성 (%) ===")
            feature_imp = sorted(zip(self.feature_names, importances), key=lambda x: x[1], reverse=True)
