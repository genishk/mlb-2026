...
class MLBBettingModel2ExtendedCatBoost:
    """
    MLB 경기 승패 예측을 위한 Model2 확장 버전 (CatBoost)
    
    mlb_model2와 동일한 기본 구조를 사용하되, 123개의 확장된 특성을 사용합니다.
    고급 기법 없이 단순한 CatBoost 분류기로 홈팀 승리 여부를 예측합니다.
    """
    ...
    def _plot_feature_importance(self, importances):
        ...
        plt.title('Top 20 Most Important Features (Model2 Extended CatBoost)')
        plt.tight_layout()
        ...
        plt.savefig(img_dir / f"feature_importance_model2_extended_catboost_{timestamp}.png")
        plt.close()
    ...
    def prepare_predictions(self, game_data: List[Dict]) -> pd.DataFrame:
        ...
        if hasattr(self.model, 'feature_count_'):
            model_feature_count = self.model.feature_count_
        elif hasattr(self.model, 'get_feature_importance'):
            model_feature_count = len(self.model.get_feature_importance())
        else:
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"Model2 확장 CatBoost 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_model2_extended_catboost_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_model2_extended_catboost_features_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {
                    'type': 'catboost_extended',
                    'params': self.model.get_params(),
                    'feature_count': len(self.feature_names)
                }
            }, f, indent=2)
        
        print(f"\n=== Model2 확장 CatBoost 모델 저장 완료 ===")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 모델 초기화 및 특성 준비
        model = MLBBettingModel2ExtendedCatBoost()
        ...
        try:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "model2_extended_catboost")
