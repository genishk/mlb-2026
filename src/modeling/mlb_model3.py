...
class MLBBettingModel3:
    """
    MLB 경기 승패 예측을 위한 세 번째 모델
    ...
    def _plot_feature_importance(self, importances):
        ...
        plt.savefig(img_dir / f"feature_importance_model3_{timestamp}.png")
        plt.close()
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model3_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features3_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            with open(pred_dir / f"mlb_game_predictions_model3_{timestamp}.json", 'w') as f:
                json.dump(predictions, f, indent=2)
            
            print(f"예측 결과 저장 완료: {pred_dir / f'mlb_game_predictions_model3_{timestamp}.json'}")
