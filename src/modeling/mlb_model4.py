...
class MLBBettingModel4:
    """
    MLB 경기 승패 예측을 위한 네 번째 모델
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        self.model = LGBMClassifier(
            n_estimators=1000,              # 트리 개수 감소 (1200 -> 500)
            learning_rate=0.02,            # 학습률 감소 (0.05 -> 0.01)
            max_depth=4,                   # 트리 깊이 감소 (4 -> 3)
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model4_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features4_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "model4")
