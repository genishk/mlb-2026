"""
MLB 총점(Over/Under) 예측 모델 4 - LightGBM Regressor (변형)
기존 mlb_model4.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModel4:
    """LightGBM 회귀 모델 (변형) - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        print("\n=== 총점 예측 모델 학습 시작 (LightGBM Regressor 변형) ===")
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_model4_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features4_{timestamp}.json"
