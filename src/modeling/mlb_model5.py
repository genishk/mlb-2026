...
class MLBBettingModel5:
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        self.model = CatBoostClassifier(
            iterations=600,               # 트리 개수 증가 (300 -> 400)
            learning_rate=0.05,          # 학습률 약간 증가 (0.01 -> 0.02)
            depth=4,                     # 트리 깊이 증가 (4 -> 5)
            l2_leaf_reg=2.0,            # L2 규제 감소 (3.0 -> 2.0)
            random_seed=42,
            loss_function='Logloss',     # 이진 분류
            eval_metric='AUC',           # AUC 측정 지표
            verbose=0,
            subsample=0.75,             # 데이터 샘플링 비율 증가 (0.8 -> 0.85)
            rsm=0.75,                   # 특성 샘플링 비율 증가 (0.8 -> 0.85)
    ...
    def _plot_feature_importance(self, importances):
        ...
        plt.savefig(img_dir / f"feature_importance_model5_{timestamp}.png")
        plt.close()
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model5_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features5_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            with open(pred_dir / f"mlb_game_predictions_model5_{timestamp}.json", 'w') as f:
                json.dump(predictions, f, indent=2)
            
            print(f"예측 결과 저장 완료: {pred_dir / f'mlb_game_predictions_model5_{timestamp}.json'}")
