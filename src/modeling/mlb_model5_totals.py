"""
MLB 총점(Over/Under) 예측 모델 5 - CatBoost Regressor (변형)
기존 mlb_model5.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModel5:
    """CatBoost 회귀 모델 (변형) - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """훈련 데이터를 사용하여 총점 예측 모델 훈련"""
        print("\n=== 총점 예측 모델 학습 시작 (CatBoost Regressor 변형) ===")
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_model5_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features5_{timestamp}.json"
