...
class MLBBettingModel7:
    """
    MLB 경기 승패 예측을 위한 일곱 번째 모델
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        self.model = LGBMClassifier(
            n_estimators=1200,              # 트리 개수 감소 (1200 -> 500)
            learning_rate=0.05,            # 학습률 감소 (0.05 -> 0.01)
            max_depth=3,                   # 트리 깊이 감소 (4 -> 3)
            num_leaves=12,                  # 리프 노드 수 감소 (70 -> 8)
            min_child_samples=25,          # 최소 샘플 수 증가 (15 -> 30)
            min_child_weight=0.1,          # 최소 가중치 증가 (0.01 -> 0.1)
            subsample=0.75,                 # 데이터 샘플링 비율 감소 (0.85 -> 0.7)
            colsample_bytree=0.75,          # 특성 샘플링 비율 감소 (0.85 -> 0.7)
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model7_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features7_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "model7")
