"""
MLB 총점(Over/Under) 예측 모델 7 - LightGBM Regressor (v3)
기존 mlb_model7.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModel7:
    """LightGBM 회귀 모델 (v3) - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        print("\n=== 총점 예측 모델 학습 시작 (LightGBM Regressor v3) ===")
    ...
    def save_model(self, timestamp=None):
        ...
        model_path = self.model_dir / f"mlb_totals_model7_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features7_{timestamp}.json"
        joblib.dump(self.model, model_path)
        with open(feature_path, 'w') as f:
            json.dump({'feature_names': self.feature_names, 'model_info': {'type': 'lightgbm_regressor_v3', 'target': 'total_score', 'params': self.model.get_params()}}, f, indent=2)
        print(f"\n=== 모델 저장 완료 ===")
    ...
    def _print_feature_importance(self):
        if self.model is None or not hasattr(self.model, 'feature_importances_'):
            return
        try:
            importances = self.model.booster_.feature_importance(importance_type='gain') if hasattr(self.model, 'booster_') else self.model.feature_importances_
