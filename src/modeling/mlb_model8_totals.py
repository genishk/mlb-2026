"""
MLB 총점(Over/Under) 예측 모델 8 - CatBoost Regressor (v3)
기존 mlb_model8.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModel8:
    """CatBoost 회귀 모델 (v3) - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        print("\n=== 총점 예측 모델 학습 시작 (CatBoost Regressor v3) ===")
    ...
    def save_model(self, timestamp=None):
        ...
        model_path = self.model_dir / f"mlb_totals_model8_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features8_{timestamp}.json"
        joblib.dump(self.model, model_path)
        with open(feature_path, 'w') as f:
            json.dump({'feature_names': self.feature_names, 'model_info': {'type': 'catboost_regressor_v3', 'target': 'total_score', 'params': self.model.get_params()}}, f, indent=2)
        print(f"\n=== 모델 저장 완료 ===")
