...
class MLBBettingModel9:
    """
    MLB 경기 승패 예측을 위한 아홉 번째 모델
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        # 하이퍼파라미터 설정 (XGBoost 특화)
        self.model = XGBClassifier(
            n_estimators=1000,             # 트리 개수 감소 (400 -> 350)
            learning_rate=0.01,          # 학습률 감소 (0.02 -> 0.015)
            max_depth=3,                  # 트리 깊이 유지
            min_child_weight=3,           # 과적합 방지 유지
            subsample=0.85,                # 데이터 샘플링 감소 (0.85 -> 0.8)
            colsample_bytree=0.85,         # 특성 샘플링 감소 (0.85 -> 0.8)
    ...
    def _plot_feature_importance(self, importances):
        ...
        plt.savefig(img_dir / f"feature_importance_model9_{timestamp}.png")
        plt.close()
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model9_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features9_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            with open(pred_dir / f"mlb_game_predictions_model9_{timestamp}.json", 'w') as f:
                json.dump(predictions, f, indent=2)
            
            print(f"예측 결과 저장 완료: {pred_dir / f'mlb_game_predictions_model9_{timestamp}.json'}")
