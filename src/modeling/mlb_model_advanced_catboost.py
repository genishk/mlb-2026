...
class MLBAdvancedCatBoostModel:
    """
    MLB 경기 승패 예측을 위한 고급 CatBoost 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 CatBoost 분류기를 사용하여 
    홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 CatBoost 모델을 훈련합니다.
        
        Args:
            training_data: 훈련 데이터 리스트
            verbose: 상세 출력 여부
        """
        print("\n=== 고급 CatBoost 모델 학습 시작 ===")
        ...
        if len(X.columns) > 50:  # 특성이 50개 이상인 경우 선택 적용
            ...
            # 데이터 수 대비 적절한 특성 수 계산 (데이터:특성 = 8:1 비율 목표)
            optimal_features = min(len(training_data) // 8, 65)  # CatBoost는 더 많은 특성 처리 가능
        ...
        # 특성 스케일링 (CatBoost는 스케일링이 덜 중요하지만 일관성을 위해 적용)
        X_scaled = self.scaler.fit_transform(X)
        X = pd.DataFrame(X_scaled, columns=X.columns, index=X.index)
        
        # 시간 가중치 (최근 데이터에 더 높은 가중치)
        n_samples = len(X)
        # CatBoost에 적합한 완만한 가중치
        sample_weights = np.linspace(0.7, 1.3, n_samples)
        sample_weights = sample_weights / sample_weights.mean()
        
        # 고급 CatBoost 모델 설정 (CatBoost에 최적화된 파라미터)
        self.model = CatBoostClassifier(
            # 기본 구조 파라미터
            iterations=700,                # 적절한 반복 수
            learning_rate=0.03,           # CatBoost 최적 학습률
            depth=5,                      # 안정적인 깊이 (과적합 방지)
            
            # CatBoost 특화 파라미터
            l2_leaf_reg=3.0,             # L2 정규화
            border_count=128,            # 수치형 특성 분할 경계 수
            
            # 과적합 방지 - 호환 가능한 설정만 사용
            random_strength=1.0,         # 랜덤성 강도
            
            # 샘플링 파라미터 (베이지안 부트스트랩과 호환)
            subsample=0.85,              # 85% 데이터 사용
            rsm=0.85,                    # 85% 특성 사용
            
            # 성능 최적화
            loss_function='Logloss',     # 이진 분류 손실
            eval_metric='AUC',           # AUC 최적화
            
            # 기타 설정
            random_seed=42,
            verbose=0,                   # 학습 과정 출력 안함
            thread_count=-1,             # 모든 CPU 코어 사용
            
            # 호환 가능한 부트스트랩 설정
            bootstrap_type='Bernoulli',  # 베르누이 부트스트랩 (subsample과 호환)
            sampling_frequency='PerTree', # 트리별 샘플링
        )
        ...
        # 특성 중요도 출력
        if verbose and hasattr(self.model, 'get_feature_importance'):
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 CatBoost 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_catboost_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_catboost_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_catboost_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_catboost_features_{timestamp}.json"
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 CatBoost 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 CatBoost 모델 초기화
    model = MLBAdvancedCatBoostModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_catboost")
