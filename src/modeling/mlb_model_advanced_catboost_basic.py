...
class MLBAdvancedCatBoostBasicModel:
    """
    MLB 경기 승패 예측을 위한 고급 특성 + 기본 파라미터 CatBoost 모델
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 특성 + 기본 파라미터 CatBoost 모델을 훈련합니다.
        
        Args:
            training_data: 훈련 데이터 리스트
            verbose: 상세 출력 여부
        """
        print("\n=== 고급 특성 + 기본 파라미터 CatBoost 모델 학습 시작 ===")
        ...
        # 시간 가중치 (mlb_model2.py와 동일)
        n_samples = len(X)
        sample_weights = np.linspace(1, 2, n_samples)
        
        # mlb_model2.py와 동일한 CatBoost 파라미터 사용
        self.model = CatBoostClassifier(
            iterations=400,               # 트리 개수 증가 (300 -> 400)
            learning_rate=0.02,          # 학습률 약간 증가 (0.01 -> 0.02)
            depth=5,                     # 트리 깊이 증가 (4 -> 5)
            l2_leaf_reg=2.0,            # L2 규제 감소 (3.0 -> 2.0)
            random_seed=42,
            loss_function='Logloss',     # 이진 분류
            eval_metric='AUC',           # AUC 측정 지표
            verbose=0,
            subsample=0.85,             # 데이터 샘플링 비율 증가 (0.8 -> 0.85)
            rsm=0.85,                   # 특성 샘플링 비율 증가 (0.8 -> 0.85)
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        print("\n=== 고급 특성 + 기본 파라미터 CatBoost 모델 예측 수행 ===")
        ...
        try:
            ...
            print(f"고급 특성 + 기본 파라미터 CatBoost 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_catboost_basic_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_catboost_basic_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_catboost_basic_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_catboost_basic_features_{timestamp}.json"
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 모델 저장 완료 ===")
    ...
    def load_model(self, model_path: str, scaler_path: str = None, 
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 모델 로드 완료 ===")
        print(f"모델: {model_path}")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 특성 + 기본 파라미터 CatBoost 모델 초기화
    model = MLBAdvancedCatBoostBasicModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_catboost_basic")
