...
class MLBTotalsAdvancedCatBoostBasicModel:
    """
    MLB 경기 총 득점 예측을 위한 고급 특성 + 기본 파라미터 CatBoost 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함하지만 기본 모델과 동일한 파라미터를 사용하는
    CatBoost 회귀기를 사용하여 경기 총 득점을 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """훈련 데이터를 사용하여 고급 특성 + 기본 파라미터 CatBoost 총 득점 예측 모델을 훈련합니다."""
        print("\n=== 고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 학습 시작 ===")
        ...
        if len(X.columns) > 50:
            ...
            optimal_features = min(len(training_data) // 8, 65)
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        print("\n=== 고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 예측 수행 ===")
        ...
        try:
            ...
            print(f"고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_advanced_catboost_basic_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_totals_advanced_catboost_basic_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_totals_advanced_catboost_basic_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_advanced_catboost_basic_features_{timestamp}.json"
        ...
        feature_info = {
            ...
            'model_info': {
                'type': 'catboost_totals_advanced_basic',
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 저장 완료 ===")
    ...
    def load_model(self, model_path: str, scaler_path: str = None,
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 로드 완료 ===")
        print(f"모델: {model_path}")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 CatBoost 총 득점 모델 성능 (최근 {len(X_recent)}경기) ===")
        print(f"MAE: {mae:.3f}")
...
if __name__ == "__main__":
    model = MLBTotalsAdvancedCatBoostBasicModel()
    try:
        ...
        if prediction_data:
            ...
            for pred in predictions:
                ...
                print()
            save_predictions(predictions, "totals_advanced_catboost_basic")
    except Exception as e:
        print(f"오류 발생: {e}")
        import traceback
        traceback.print_exc()
