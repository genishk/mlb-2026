...
class MLBAdvancedBettingModel:
    """
    MLB 경기 승패 예측을 위한 고급 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 LightGBM 분류기를 사용하여 
    홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 승패 예측 모델을 훈련합니다.
        ...
        if len(X.columns) > 50:  # 특성이 50개 이상인 경우 선택 적용
            ...
            optimal_features = min(len(training_data) // 8, 60)  # 최대 60개 특성
        ...
        n_samples = len(X)
        # 지수적 증가보다 완만한 선형 증가 사용
        sample_weights = np.linspace(0.5, 1.5, n_samples)
        sample_weights = sample_weights / sample_weights.mean()
        
        # 고급 LightGBM 모델 설정 (더 최적화된 파라미터)
        self.model = LGBMClassifier(
            # 기본 구조 파라미터
            n_estimators=800,              # 적절한 트리 개수
            learning_rate=0.02,            # 안정적인 학습률
            max_depth=5,                   # 적절한 깊이
            num_leaves=31,                 # 2^max_depth - 1 (최적 비율)
            
            # 과적합 방지 파라미터
            min_child_samples=20,          # 과적합 방지 강화
            min_child_weight=0.001,        # 적절한 가중치
            min_split_gain=0.02,           # 분할 최소 이득 증가
            
            # 샘플링 파라미터 (배깅 효과)
            subsample=0.85,                # 85% 데이터 사용
            subsample_freq=1,              # 매 트리마다 샘플링
            colsample_bytree=0.85,         # 85% 특성 사용 (feature_fraction과 동일)
            
            # 정규화 파라미터
            reg_alpha=0.1,                 # L1 규제 (특성 선택 효과)
            reg_lambda=0.8,                # L2 규제 강화
            ...
            # 추가 설정 (중복 제거)
            bagging_fraction=0.85,         # subsample과 동일
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_lgbm_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_lgbm_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_lgbm_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_lgbm_features_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_model")
