...
class MLBAdvancedLGBMBasicModel:
    """
    MLB 경기 승패 예측을 위한 고급 특성 + 기본 파라미터 LightGBM 모델
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 특성 + 기본 파라미터 LightGBM 모델을 훈련합니다.
        
        Args:
            training_data: 훈련 데이터 리스트
            verbose: 상세 출력 여부
        """
        print("\n=== 고급 특성 + 기본 파라미터 LightGBM 모델 학습 시작 ===")
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        print("\n=== 고급 특성 + 기본 파라미터 LightGBM 모델 예측 수행 ===")
        ...
        try:
            ...
            print(f"고급 특성 + 기본 파라미터 LightGBM 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_lgbm_basic_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_lgbm_basic_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_lgbm_basic_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_lgbm_basic_features_{timestamp}.json"
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 LightGBM 모델 저장 완료 ===")
    ...
    def load_model(self, model_path: str, scaler_path: str = None, 
        ...
        print(f"\n=== 고급 특성 + 기본 파라미터 LightGBM 모델 로드 완료 ===")
        print(f"모델: {model_path}")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 특성 + 기본 파라미터 LightGBM 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 특성 + 기본 파라미터 LightGBM 모델 초기화
    model = MLBAdvancedLGBMBasicModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_lgbm_basic")
