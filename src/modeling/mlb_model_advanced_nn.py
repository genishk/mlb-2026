...
class MLBAdvancedNeuralNetworkModel:
    """
    MLB 경기 승패 예측을 위한 고급 Neural Network 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 MLP 분류기를 사용하여 
    홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 Neural Network 모델을 훈련합니다.
        
        Args:
            training_data: 훈련 데이터 리스트
            verbose: 상세 출력 여부
        """
        print("\n=== 고급 Neural Network 모델 학습 시작 ===")
        ...
        if len(X.columns) > 50:  # 특성이 50개 이상인 경우 선택 적용
            ...
            optimal_features = min(len(training_data) // 8, 50)  # 최대 60개 특성 (50→60 증가)
        ...
        # 특성 스케일링 (Neural Network에 매우 중요)
        X_scaled = self.scaler.fit_transform(X)
        X = pd.DataFrame(X_scaled, columns=X.columns, index=X.index)
        
        # 시간 가중치 (최근 데이터 중시 강화)
        n_samples = len(X)
        sample_weights = np.linspace(0.6, 1.4, n_samples)  # 가중치 범위 확대 (0.8-1.2 → 0.6-1.4)
        sample_weights = sample_weights / sample_weights.mean()
        
        # 과적합 강화 Neural Network 모델 설정 (복잡한 패턴 학습)
        self.model = MLPClassifier(
            # 네트워크 구조 (더 깊고 넓은 구조로 복잡성 증가)
            hidden_layer_sizes=(150, 100, 75, 50, 25),  # 5층 구조로 더 깊게 (3층→5층)
            ...
            # 정규화 파라미터 (과적합 허용)
            alpha=0.001,                   # L2 정규화 감소 (0.005→0.001, 과적합 허용)
            
            # 학습 파라미터 (더 정밀한 학습)
            learning_rate='adaptive',      # 적응적 학습률
            learning_rate_init=0.005,      # 초기 학습률 증가 (0.003→0.005)
            max_iter=800,                  # 최대 반복 횟수 증가 (400→800)
            
            # 배치 및 샘플링
            batch_size=16,                 # 배치 크기 감소 (32→16, 더 세밀한 학습)
            shuffle=True,                  # 에포크마다 셔플
            
            # 조기 종료 (완화하여 더 오래 학습)
            early_stopping=True,           # 조기 종료 활성화
            validation_fraction=0.1,       # 검증 데이터 비율 감소 (0.15→0.1)
            n_iter_no_change=25,          # 조기 종료 기준 완화 (15→25)
            tol=1e-5,                     # 허용 오차 감소 (1e-4→1e-5)
            ...
            # 기타 설정
            random_state=42,
            warm_start=False,             # 이전 학습 결과 사용 안함
            max_fun=25000                 # 최대 함수 호출 횟수 증가 (15000→25000)
        )
        ...
        # 최종 모델 훈련 (시간 가중치 강화)
        if len(X) > 100:
            # 최근 30% 데이터를 두 번 더 추가하여 시간 가중치 효과 강화
            recent_size = int(len(X) * 0.3)  # 20% → 30% 증가
            recent_X = X.iloc[-recent_size:]
            recent_y = y.iloc[-recent_size:]
            
            # 원본 + 최근 데이터 2번 결합 (더 강한 가중치)
            X_weighted = pd.concat([X, recent_X, recent_X], ignore_index=True)
            y_weighted = pd.concat([y, recent_y, recent_y], ignore_index=True)
            
            print(f"시간 가중치 강화: 원본 {len(X)} + 최근 {len(recent_X)}×2 = 총 {len(X_weighted)}개 샘플")
            self.model.fit(X_weighted, y_weighted)
        else:
            self.model.fit(X, y)
        
        print(f"학습 완료 - 반복 횟수: {self.model.n_iter_}")
        print(f"최종 손실: {self.model.loss_:.6f}")
        
        # 특성 중요도 출력 (Neural Network 근사)
        if verbose:
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 Neural Network 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_nn_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_nn_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_nn_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_nn_features_{timestamp}.json"
        ...
        feature_info = {
            ...
            'model_info': {
                'type': 'neural_network_advanced',
                'params': self.model.get_params(),
                'feature_selection': self.feature_selector is not None,
                'scaling': 'robust',
                'network_structure': f"{len(self.selected_features)} -> 150 -> 100 -> 75 -> 50 -> 25",
    ...
    def load_model(self, model_path: str, scaler_path: str = None, 
        ...
        print(f"\n=== 고급 Neural Network 모델 로드 완료 ===")
        print(f"모델: {model_path}")
    
    def _plot_feature_importance(self) -> None:
        """Neural Network의 가중치를 기반으로 특성 중요도 근사 출력"""
        if self.model is None or not hasattr(self.model, 'coefs_'):
            return
            
        try:
            # 첫 번째 은닉층의 가중치를 사용하여 특성 중요도 계산
            first_layer_weights = self.model.coefs_[0]  # (n_features, n_hidden)
            
            # 각 특성에 대한 가중치의 절댓값 평균을 계산
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 Neural Network 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 Neural Network 모델 초기화
    model = MLBAdvancedNeuralNetworkModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_nn")
