...
class MLBAdvancedRandomForestModel:
    """
    MLB 경기 승패 예측을 위한 고급 Random Forest 모델
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 Random Forest 모델을 훈련합니다.
        
        Args:
            training_data: 훈련 데이터 리스트
            verbose: 상세 출력 여부
        """
        print("\n=== 고급 Random Forest 모델 학습 시작 ===")
        ...
        if len(X.columns) > 50:  # 특성이 50개 이상인 경우 선택 적용
            ...
            # 데이터 수 대비 적절한 특성 수 계산 (Random Forest 최적화)
            optimal_features = min(len(training_data) // 5, 70)  # 최대 70개 특성 (많은 특성 처리 가능)
        ...
        # 특성 스케일링 (Random Forest는 스케일링이 덜 중요하지만 일관성을 위해 적용)
        X_scaled = self.scaler.fit_transform(X)
        X = pd.DataFrame(X_scaled, columns=X.columns, index=X.index)
        
        # 시간 가중치 (Random Forest에 적합한 중간 정도 가중치)
        n_samples = len(X)
        sample_weights = np.linspace(0.6, 1.4, n_samples)
        sample_weights = sample_weights / sample_weights.mean()
        
        # Random Forest 모델 (고급 설정)
        rf_model = RandomForestClassifier(
            n_estimators=400,              # 트리 개수 증가
            max_depth=12,                  # 깊이 증가 (고급 특성 상호작용 포착)
        ...
        # Extra Trees 모델 (고급 설정)
        et_model = ExtraTreesClassifier(
            n_estimators=400,              # 트리 개수 증가
            max_depth=15,                  # 더 깊은 트리 (Extra Trees 특성)
            min_samples_split=6,           # 분할을 위한 최소 샘플 수
        ...
        # OOB 점수 출력 (Random Forest만)
        if hasattr(self.model.named_estimators_['rf'], 'oob_score_'):
            print(f"Random Forest OOB 점수: {self.model.named_estimators_['rf'].oob_score_:.3f}")
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 Random Forest 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_rf_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_rf_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_rf_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_rf_features_{timestamp}.json"
    ...
    def _plot_feature_importance(self) -> None:
        ...
        try:
            # Random Forest와 Extra Trees의 특성 중요도 계산
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 Random Forest 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 Random Forest 모델 초기화
    model = MLBAdvancedRandomForestModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_rf")
