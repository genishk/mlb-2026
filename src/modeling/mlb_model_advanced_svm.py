...
class MLBAdvancedSVMModel:
    """
    MLB 경기 승패 예측을 위한 고급 SVM 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 Support Vector Machine을 사용하여 
    홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        # 특성 선택 (상위 K개 특성 선택) - SVM은 특성 수에 민감
        if len(X.columns) > 25:  # 특성이 25개 이상인 경우 선택 적용 (30→25로 낮춰서 더 자주 적용)
            print("특성 선택 수행 중...")
            ...
            # 데이터 수 대비 적절한 특성 수 계산 (더 많은 특성 사용)
            optimal_features = min(len(training_data) // 6, 60)  # 최대 60개 특성 (45→60 증가, 비율도 9→6으로 증가)
        ...
        # 시간 가중치 (최근 데이터 중시 더욱 강화)
        n_samples = len(X)
        sample_weights = np.linspace(0.5, 1.5, n_samples)  # 가중치 범위 더 확대 (0.6-1.4 → 0.5-1.5)
        sample_weights = sample_weights / sample_weights.mean()
        
        # 과적합 강화 SVM 모델 설정 (복잡한 패턴 학습)
        self.model = SVC(
            # 커널 설정
            kernel='rbf',                  # RBF 커널 (비선형 패턴 포착)
            
            # 정규화 파라미터 (과적합 허용으로 복잡한 패턴 학습)
            C=15.0,                       # 정규화 강도 대폭 증가 (5.0→15.0, 더 복잡한 경계 허용)
            gamma=0.5,                    # RBF 커널 계수 대폭 증가 (0.2→0.5, 더 세밀하고 확실한 경계)
            ...
            # 수치적 안정성 (더 정밀한 학습)
            tol=1e-7,                    # 허용 오차 더 감소 (1e-6→1e-7, 매우 정밀한 수렴)
            max_iter=5000,               # 최대 반복 횟수 대폭 증가 (3000→5000)
            
            # 기타 설정
            random_state=42,
            shrinking=False,             # 휴리스틱 최적화 비활성화 (더 정밀한 학습)
            cache_size=800,              # 캐시 크기 대폭 증가 (500→800MB)
        ...
        # 확률 캘리브레이션 적용 (더 확실한 예측을 위해)
        print("확률 캘리브레이션 수행 중...")
        calibrated_model = CalibratedClassifierCV(
            self.model, 
            method='isotonic',  # 비모수적 캘리브레이션 (더 유연)
            cv=3,              # 3-fold CV
            ensemble=True      # 앙상블 방식으로 성능 향상
        )
        calibrated_model.fit(X, y, sample_weight=sample_weights)
        self.model = calibrated_model  # 캘리브레이션된 모델로 교체
        
        print(f"SVM 학습 완료 - 확률 캘리브레이션 적용됨")
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 SVM 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_svm_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_svm_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_svm_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_svm_features_{timestamp}.json"
        ...
        try:
            # 기본 파라미터만 저장 (JSON 직렬화 가능한 것들만)
            model_params = {
                'kernel': getattr(base_model, 'kernel', 'rbf'),
                'C': getattr(base_model, 'C', 1.0),
                'gamma': getattr(base_model, 'gamma', 'scale'),
                'class_weight': str(getattr(base_model, 'class_weight', None)),
                'probability': getattr(base_model, 'probability', True),
                'random_state': getattr(base_model, 'random_state', None),
                'max_iter': getattr(base_model, 'max_iter', 1000)
            }
        except:
            model_params = {'type': 'calibrated_svm'}
        ...
        feature_info = {
            ...
            'model_info': {
                ...
                'scaling': 'robust',
                'calibrated': hasattr(self.model, 'calibrated_classifiers_'),
                'n_support_vectors': getattr(base_model, 'n_support_', []).tolist() if hasattr(base_model, 'n_support_') else 'N/A',
                'support_vector_ratio': 'N/A'  # 캘리브레이션된 모델에서는 정확한 비율 계산 불가
            }
        }
    ...
    def _plot_feature_importance(self) -> None:
        """SVM의 가중치를 기반으로 특성 중요도 근사 출력"""
        if self.model is None or not hasattr(self.model, 'coef_'):
            # RBF 커널의 경우 coef_가 없으므로 permutation importance 근사
            print("\n=== SVM 특성 중요도 (근사) ===")
            print("RBF 커널 SVM은 직접적인 특성 중요도를 제공하지 않습니다.")
            print("선택된 특성들이 모델 성능에 기여하고 있습니다.")
            
            # 선택된 특성 목록 출력
            if self.selected_features:
                print(f"\n선택된 {len(self.selected_features)}개 특성:")
                for i, feature in enumerate(self.selected_features[:20], 1):
                    print(f"{i:2d}. {feature}")
            return
            
        try:
            # 선형 커널의 경우에만 가중치 기반 중요도 계산 가능
            if self.model.kernel == 'linear':
                # 가중치의 절댓값을 특성 중요도로 사용
                ...
                print("\n=== SVM 특성 중요도 (근사) ===")
                print("비선형 커널 SVM은 직접적인 특성 중요도를 제공하지 않습니다.")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 SVM 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_svm")
