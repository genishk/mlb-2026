...
class MLBTotalsAdvancedSVMModel:
    """
    MLB 경기 총 득점 예측을 위한 고급 SVR 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 Support Vector Regression을 사용하여
    경기 총 득점을 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        ...
        if len(X.columns) > 25:
            ...
            optimal_features = min(len(training_data) // 6, 60)
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 SVR 총 득점 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_advanced_svm_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_totals_advanced_svm_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_totals_advanced_svm_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_advanced_svm_features_{timestamp}.json"
        ...
        try:
            ...
            model_params = {'type': 'svr'}
            
        feature_info = {
            'feature_names': list(self.feature_names) if self.feature_names else [],
            'selected_features': list(self.selected_features) if self.selected_features else [],
            'model_info': {
                'type': 'svr_totals_advanced',
                'params': model_params,
                'feature_selection': self.feature_selector is not None,
                'scaling': 'robust',
                'n_support_vectors': getattr(self.model, 'n_support_', []).tolist() if hasattr(getattr(self.model, 'n_support_', []), 'tolist') else [],
            }
        }
        with open(feature_path, 'w') as f:
            json.dump(feature_info, f, indent=2)
        print(f"\n=== 고급 SVR 총 득점 모델 저장 완료 ===")
    ...
    def load_model(self, model_path: str, scaler_path: str = None,
        ...
        print(f"\n=== 고급 SVR 총 득점 모델 로드 완료 ===")
    
    def _plot_feature_importance(self) -> None:
        print("\n=== SVR 특성 중요도 (근사) ===")
        print("RBF 커널 SVR은 직접적인 특성 중요도를 제공하지 않습니다.")
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        print(f"\n=== 고급 SVR 총 득점 모델 성능 (최근 {len(X_recent)}경기) ===")
        print(f"MAE: {mae:.3f}")
...
if __name__ == "__main__":
    model = MLBTotalsAdvancedSVMModel()
    try:
        ...
        if prediction_data:
            ...
            save_predictions(predictions, "totals_advanced_svm")
    except Exception as e:
        print(f"오류 발생: {e}")
        import traceback
        traceback.print_exc()
