...
class MLBAdvancedXGBoostModel:
    """
    MLB 경기 승패 예측을 위한 고급 XGBoost 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 XGBoost 분류기를 사용하여 
    홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        """
        훈련 데이터를 사용하여 고급 XGBoost 모델을 훈련합니다.
        ...
        n_samples = len(X)
        # XGBoost에 적합한 중간 정도 가중치
        sample_weights = np.linspace(0.6, 1.4, n_samples)
        sample_weights = sample_weights / sample_weights.mean()
        
        # 고급 XGBoost 모델 설정 (XGBoost에 최적화된 파라미터)
        self.model = XGBClassifier(
            # 기본 구조 파라미터
            n_estimators=600,             # 적절한 트리 개수
            learning_rate=0.025,          # 안정적인 학습률
            max_depth=5,                  # 안정적인 깊이 (과적합 방지)
            min_child_weight=3,           # 과적합 방지 강화
            
            # 샘플링 파라미터
            subsample=0.85,               # 85% 데이터 사용
            colsample_bytree=0.85,        # 85% 특성 사용
            colsample_bylevel=0.85,       # 레벨별 특성 샘플링
            
            # 정규화 파라미터
            reg_alpha=0.2,                # L1 규제 (적절히 조정)
            reg_lambda=1.2,               # L2 규제 (적절히 조정)
            gamma=0.1,                    # 최소 분할 손실
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 XGBoost 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 파일 경로들
        model_path = self.model_dir / f"mlb_advanced_xgboost_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_advanced_xgboost_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_advanced_xgboost_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_advanced_xgboost_features_{timestamp}.json"
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        # 결과 출력
        print(f"\n=== 고급 XGBoost 모델 성능 (최근 {len(X_recent)}경기) ===")
...
if __name__ == "__main__":
    # MLB 고급 XGBoost 모델 초기화
    model = MLBAdvancedXGBoostModel()
    ...
    try:
        ...
        if prediction_data:
            ...
            # 예측 결과 저장
            save_predictions(predictions, "advanced_xgboost")
