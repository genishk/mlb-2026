...
class MLBTotalsAdvancedXGBoostModel:
    """
    MLB 경기 총 득점 예측을 위한 고급 XGBoost 모델
    
    고급 특성 엔지니어링과 특성 선택을 포함한 XGBoost 회귀기를 사용하여
    경기 총 득점을 예측합니다.
    """
    ...
    def predict(self, game_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            print(f"고급 XGBoost 총 득점 모델 예측 완료: {len(results)}개 경기")
            return results
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_advanced_xgboost_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_totals_advanced_xgboost_scaler_{timestamp}.joblib"
        selector_path = self.model_dir / f"mlb_totals_advanced_xgboost_selector_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_advanced_xgboost_features_{timestamp}.json"
    ...
    def evaluate_model(self, X: pd.DataFrame, y: pd.Series, n_games: int = 50) -> Dict:
        ...
        print(f"\n=== 고급 XGBoost 총 득점 모델 성능 (최근 {len(X_recent)}경기) ===")
        print(f"MAE: {mae:.3f}")
...
if __name__ == "__main__":
    model = MLBTotalsAdvancedXGBoostModel()
    try:
        ...
        if prediction_data:
            ...
            save_predictions(predictions, "totals_advanced_xgboost")
    except Exception as e:
        print(f"오류 발생: {e}")
        import traceback
        traceback.print_exc()
