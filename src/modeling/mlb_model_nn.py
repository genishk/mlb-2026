...
class MLBBettingModelNN:
    """
    MLB 경기 승패 예측을 위한 Neural Network 모델
    
    Multi-layer Perceptron (MLP) 분류기를 사용하여 홈팀 승리 여부를 예측합니다.
    """
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        sample_weights = np.linspace(0.7, 1.3, n_samples)
        
        # 특성 스케일링 (Neural Network에 중요)
        X_scaled = self.scaler.fit_transform(X)
        
        # Neural Network 모델 설정 (90%대 균형잡힌 성능 목표)
        # 과적합 방지를 위한 적절한 복잡도 조정
        self.model = MLPClassifier(
            hidden_layer_sizes=(128, 64, 32),  # 3개 은닉층으로 축소: 128->64->32 (과적합 방지)
            activation='relu',                  # ReLU 활성화 함수
            solver='adam',                      # Adam 옵티마이저
            alpha=0.01,                        # L2 규제 강도 증가 (0.0001 -> 0.01) - 과적합 방지
            batch_size=64,                     # 배치 크기 증가 (32 -> 64) - 일반화 성능 향상
            learning_rate='adaptive',          # 적응적 학습률
            learning_rate_init=0.005,          # 초기 학습률 감소 (0.01 -> 0.005) - 안정적인 학습
            max_iter=300,                      # 최대 반복 횟수 감소 (1000 -> 300) - 과적합 방지
            shuffle=True,                      # 에포크마다 데이터 셔플
            random_state=42,
            early_stopping=True,               # 조기 종료
            validation_fraction=0.2,           # 검증 데이터 비율 증가 (0.15 -> 0.2) - 더 엄격한 검증
            n_iter_no_change=10,              # 조기 종료 기준 강화 (25 -> 10) - 빠른 조기 종료
            tol=1e-3,                         # 허용 오차 증가 (1e-5 -> 1e-3) - 과적합 방지
            warm_start=False,                 # 이전 학습 결과 사용 안함
            momentum=0.9,                     # 모멘텀 감소 (0.95 -> 0.9)
            nesterovs_momentum=True,          # Nesterov 모멘텀
            beta_1=0.9,                       # Adam beta1
            beta_2=0.999,                     # Adam beta2
            epsilon=1e-8,                     # Adam epsilon
            max_fun=15000                     # 최대 함수 호출 횟수 감소 (50000 -> 15000)
        )
        
        print("\n=== Neural Network 모델 학습 시작 (90%대 균형잡힌 성능 목표) ===")
        print(f"네트워크 구조: {X.shape[1]} -> 128 -> 64 -> 32 -> 1")
        print("과적합 방지를 위한 적절한 복잡도 + 적당한 시간 가중치")
        
        # 모델 학습 (MLPClassifier는 sample_weight를 지원하지 않음)
        # 적당한 시간 가중치로 과적합 방지
        if len(X_scaled) > 100:  # 충분한 데이터가 있는 경우에만 적용
            # 최근 20% 데이터를 1번만 추가 샘플링 (적당한 가중치)
            recent_size = int(len(X_scaled) * 0.2)
            recent_X = X_scaled[-recent_size:]
            recent_y = y.iloc[-recent_size:].values
            
            # 원본 데이터 + 최근 데이터 1번 결합 (과적합 방지)
            X_weighted = np.vstack([
                X_scaled,           # 원본 데이터
                recent_X            # 최근 20% 데이터 (1번만 추가)
            ])
            y_weighted = np.concatenate([
                y.values,           # 원본 레이블
                recent_y            # 최근 레이블 (1번만 추가)
            ])
            
            print(f"적당한 가중치 적용: 원본 {len(X_scaled)} + 최근 {len(recent_X)} = 총 {len(X_weighted)}개 샘플")
            self.model.fit(X_weighted, y_weighted)
        else:
            # 데이터가 적으면 그냥 원본 데이터로 학습
            self.model.fit(X_scaled, y)
        
        print(f"학습 완료 - 반복 횟수: {self.model.n_iter_}")
        print(f"최종 손실: {self.model.loss_:.6f}")
        
        # 특성 중요도는 Neural Network에서 직접 계산하기 어려우므로
        # 가중치 기반으로 근사적으로 계산
        ...
        metrics = {
            ...
            'network_structure': f"{X.shape[1]} -> 128 -> 64 -> 32 -> 1"
        }
    ...
    def _calculate_feature_importance(self, X_scaled: np.ndarray) -> np.ndarray:
        """Neural Network의 가중치를 기반으로 특성 중요도 근사 계산"""
    ...
    def save_model(self, timestamp: str = None) -> str:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"mlb_betting_model_nn_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_scaler_nn_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_features_nn_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {
                    ...
                    'network_structure': f"{len(self.feature_names)} -> 128 -> 64 -> 32 -> 1",
                    'n_iterations': self.model.n_iter_,
                    'final_loss': self.model.loss_
                }
            }, f, indent=2)
    ...
    try:
        ...
        try:
            ...
            if prediction_data:
                ...
                with open(pred_dir / f"mlb_game_predictions_model_nn_{timestamp}.json", 'w') as f:
                    json.dump(predictions, f, indent=2)
                
                print(f"예측 결과 저장 완료: {pred_dir / f'mlb_game_predictions_model_nn_{timestamp}.json'}")
