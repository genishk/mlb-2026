"""
MLB 총점(Over/Under) 예측 모델 - MLPRegressor + StandardScaler
"""
import json
...
class MLBTotalsModelNN:
    """MLP 회귀 + StandardScaler - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        print("\n=== 총점 예측 모델 학습 시작 (MLPRegressor + StandardScaler) ===")
        ...
        if len(X_scaled) > 100:
            ...
            X_fit = np.vstack([X_scaled, recent_X])
            y_fit = np.concatenate([y.values, recent_y])
            print(
                f"최근 데이터 보강: 원본 {len(X_scaled)} + 최근 20% {len(recent_X)} = 총 {len(X_fit)}개 샘플"
            )
        ...
        if verbose:
            print(f"학습 완료 - 반복 횟수: {self.model.n_iter_}, 최종 손실: {self.model.loss_:.6f}")
    ...
    def _print_feature_importance(self) -> None:
        if self.model is None or not self.feature_names:
            return
        first = self.model.coefs_[0]
        imp = np.mean(np.abs(first), axis=1)
        imp = 100.0 * (imp / imp.sum())
        print("\n=== 상위 15개 중요 특성 (1층 가중치 근사 %) ===")
        for feat, v in sorted(zip(self.feature_names, imp), key=lambda x: x[1], reverse=True)[:15]:
            print(f"{feat}: {v:.2f}%")
    ...
    def save_model(self, timestamp: Optional[str] = None) -> str:
        ...
        model_path = self.model_dir / f"mlb_totals_model_nn_{timestamp}.joblib"
        scaler_path = self.model_dir / f"mlb_totals_scaler_nn_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features_nn_{timestamp}.json"
    ...
    def _infer_paths_from_model_path(self, model_path: str) -> Tuple[Optional[Path], Optional[Path]]:
        p = Path(model_path)
        stem = p.stem
        if "model_nn_" in stem:
            ts = stem.split("model_nn_", 1)[-1]
            return (
                p.parent / f"mlb_totals_scaler_nn_{ts}.joblib",
                p.parent / f"mlb_totals_features_nn_{ts}.json",
            )
        return None, None
