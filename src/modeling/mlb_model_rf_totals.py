"""
MLB 총점(Over/Under) 예측 모델 RF - Random Forest + Extra Trees Regressor 앙상블
기존 mlb_model_rf.py 기반, 타겟만 total_score로 변경
"""
import pandas as pd
import numpy as np
...
class MLBTotalsModelRF:
    """Random Forest + Extra Trees 회귀 앙상블 - MLB 총점 예측"""
    ...
    def train_model(self, training_data: List[Dict], verbose: bool = True) -> None:
        print("\n=== 총점 예측 모델 학습 시작 (RF + ET Regressor 앙상블) ===")
        ...
        et_model = ExtraTreesRegressor(
            ...
            min_samples_leaf=3, max_features='sqrt', bootstrap=False,
            random_state=42, n_jobs=-1
        )
        ...
        if verbose:
            ...
            et_imp = self.model.named_estimators_['et'].feature_importances_
            avg_imp = 100.0 * ((rf_imp + et_imp) / 2) / ((rf_imp + et_imp) / 2).sum()
            print("\n=== 상위 15개 중요 특성 (평균 %) ===")
            for feat, imp in sorted(zip(self.feature_names, avg_imp), key=lambda x: x[1], reverse=True)[:15]:
                print(f"{feat}: {imp:.2f}%")
    ...
    def evaluate_model(self, X, y, n_games=50):
        ...
        print(f"MAE: {mae:.2f}점 | RMSE: {rmse:.2f}점 | R²: {r2:.3f}")
        errors = y_pred - y_recent.values
        print(f"±1점 이내: {np.sum(np.abs(errors)<=1)/len(errors)*100:.1f}% | ±2점: {np.sum(np.abs(errors)<=2)/len(errors)*100:.1f}% | ±3점: {np.sum(np.abs(errors)<=3)/len(errors)*100:.1f}%")
        if hasattr(self, 'dates') and self.dates is not None:
            dates_recent = self.dates[-len(X_recent):]
            print(f"\n=== 최근 10경기 예측 상세 ===")
            print(f"{'날짜':<12} {'실제':>6} {'예측':>6} {'오차':>6}")
            print("-" * 35)
            for i in range(max(0, len(X_recent)-10), len(X_recent)):
                print(f"{dates_recent.iloc[i].strftime('%Y-%m-%d'):<12} {y_recent.iloc[i]:>6.0f} {y_pred[i]:>6.1f} {y_pred[i]-y_recent.iloc[i]:>+6.1f}")
        return {'mae': mae, 'rmse': rmse, 'r2': r2}

    def predict(self, game_data):
        if self.model is None: raise ValueError("모델이 학습되지 않았습니다.")
        if not self.feature_names: raise ValueError("특성 이름이 로드되지 않았습니다.")
    ...
    def prepare_predictions(self, game_data):
        ...
        if null_cols: raise ValueError(f"예측 데이터에 결측치가 있는 특성: {null_cols}")
        return X

    def save_model(self, timestamp=None):
        if self.model is None: raise ValueError("저장할 모델이 없습니다.")
        if timestamp is None: timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_path = self.model_dir / f"mlb_totals_model_rf_{timestamp}.joblib"
        feature_path = self.model_dir / f"mlb_totals_features_rf_{timestamp}.json"
        joblib.dump(self.model, model_path)
        with open(feature_path, 'w') as f:
            json.dump({'feature_names': self.feature_names, 'model_info': {'type': 'rf_et_regressor_ensemble', 'target': 'total_score'}}, f, indent=2)
        print(f"\n=== 모델 저장 완료 ===\n모델: {model_path}\n특성: {feature_path}")
...
def get_latest_training_data():
    ...
    if not training_dir.exists(): raise FileNotFoundError(f"학습 데이터 디렉토리가 없습니다: {training_dir}")
    json_files = list(training_dir.glob('mlb_training_data_*.json'))
    if not json_files: raise FileNotFoundError("학습 데이터 파일을 찾을 수 없습니다.")
