...
def run_for_tag(model_tag: str):
    """특정 태그의 모델로 앙상블 예측 실행"""
    print(f"\n{'='*60}")
    print(f"  [{model_tag.upper()}] 앙상블 예측 시작")
    print(f"{'='*60}")
    ...
    print(f"\n=== [{model_tag.upper()}] 예측 저장 완료: {output_path} ===")
    return output_path


def main():
    """MLB 앙상블 예측 모델 실행 함수"""
    parser = argparse.ArgumentParser(description='MLB 앙상블 예측 실행')
    ...
    try:
        ...
        print(f"프로젝트 루트: {script_path.parent.parent.parent}")
        print(f"모델 태그: {args.model_tag}")
