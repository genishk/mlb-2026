...
<style>
    ...
    /* 스피너와 로딩 상태 개선 */
    .stSpinner > div > div {
        border-color: #3b82f6 #e5e7eb #e5e7eb #e5e7eb !important;
    }
    ...
    @media (max-width: 768px) {
        ...
        /* 텔레그램 채널 설명 텍스트들 더 축소 */
        ul li, .stMarkdown li {
            font-size: 0.75rem !important;
            line-height: 1.3 !important;
        }
        
        /* 메트릭 라벨 bold 처리 + 크기 키우기 */
        ...
        /* 모바일에서도 Data Updates 섹션 폰트 MUCH 더 크게! */
        .stInfo,
        .stInfo div,
        .stInfo > div,
        .stInfo p,
        ...
        /* 차트들 모바일에서 세로 배치 - 안전한 방식 */
        .stColumns > div > div:has(.plotly-graph-div) {
            width: 100% !important;
            margin-bottom: 1rem !important;
        }
        ...
        /* About This Tracker 섹션 설명들 크기 키우기 */
        .stMarkdown ul li,
        .stMarkdown li {
            font-size: 0.9rem !important;
        ...
        /* Join Our Community 섹션 전체 간격 조정 */
        .stMarkdown h3 {
            margin-bottom: 0.5rem !important;
        }
        ...
        /* Data Updates 섹션 st.info 폰트 크기 MUCH 더 크게! */
        .stInfo,
        .stInfo div,
        .stInfo > div,
        .stInfo p,
        ...
        /* Daily Picks Performance 테이블 모바일 최적화 */
        .daily-picks-table {
            font-size: 0.5rem !important;
        }
        ...
        /* Daily Picks 차트 높이 모바일 최적화 */
        .daily-picks-chart .plotly-graph-div {
            height: 180px !important;
        }
    }
</style>
...
def analyze_predicted_roi_segments(analysis_data):
    ...
    for data in analysis_data:
        ...
        if pred_roi < -20:
            ...
            segments['Very Positive (>20%)'].append(data)
...
def analyze_odds_segments(analysis_data):
    ...
    for data in analysis_data:
        ...
        if odds < -200:
            ...
            segments['Underdog (+120 ~ +300)'].append(data)
        else:
...
def analyze_kelly_segments(analysis_data):
    ...
    for data in analysis_data:
        ...
        if kelly_percentage <= 0:
            ...
            segments['Very High Confidence (Kelly > 25%)'].append(data)
...
def main():
    ...
    **🔔 Get Real-time Updates & Analysis**
    ...
    <div class="pick-detail">
        <span style="color: #00ff88; font-size: 1.1rem; font-weight: bold;">#1 Seattle Mariners @ Detroit Tigers</span><br>
        🎯 Prediction Result: Seattle Mariners (-108.0)<br>
        💎 Statistical Probability: 62.8% | 📊 ROI Projection: +20.9% | 💡 Confidence Level: 0.128<br>
    ...
    st.markdown("""
    <div class="pick-detail">
        <span style="color: #00ff88; font-size: 1.1rem; font-weight: bold;">#2 Atlanta Braves @ St. Louis Cardinals</span><br>
        🎯 Prediction Result: Atlanta Braves (+136.0)<br>
        💎 Statistical Probability: 52.8% | 📊 ROI Projection: +24.7% | 💡 Confidence Level: 0.028<br>
    ...
    st.markdown("""
    <div class="pick-detail">
        <span style="color: #00ff88; font-size: 1.1rem; font-weight: bold;">#3 Los Angeles Dodgers @ San Francisco Giants</span><br>
        🎯 Prediction Result: Los Angeles Dodgers (-145.0)<br>
        💎 Statistical Probability: 68.4% | 📊 ROI Projection: +18.2% | 💡 Confidence Level: 0.184<br>
    ...
    if start_date <= end_date:
        with st.spinner("📊 Analyzing model performance..."):
            try:
                ...
                with col3:
                    st.metric("Models Tracked", models_count)
                with col4:
                ...
                if best_model_name and compound_data:
                    ...
                    <div class="metric-container" style="margin: 0.5rem 0 1rem 0;">
                       <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); 
                           ...
                           <div style="font-size: clamp(2rem, 7vw, 2.5rem); font-weight: 900; 
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("ROI", f"{best_model['ROI (%)']:.2f}%")
                with col2:
                    st.metric("Win Rate", f"{best_model['Win Rate (%)']:.1f}%")
                with col3:
                    st.metric("Total Predictions", f"{best_model['Total Predictions']}")
                with col4:
                    st.metric("Profit/Loss", f"${best_model['Profit/Loss ($)']:.1f}")
                    ...
                    if selected_model in daily_data:
                       ...
                       if not daily_df.empty:
                           ...
                           with col2:
                               ...
                               st.metric("Profitable Days", f"{profitable_days} ({profitable_days/len(daily_df)*100:.1f}%)")
                           with col3:
                               st.metric("Avg Daily ROI", f"{daily_df['ROI (%)'].mean():.2f}%")
                           with col4:
                               st.metric("Total P/L", f"${daily_df['Profit/Loss ($)'].sum():.1f}")
                           ...
                           with col1:
                               ...
                               st.metric("Best Day ROI", f"{best_day_roi:.2f}%")
                           with col2:
                               worst_day_roi = daily_df['ROI (%)'].min()
                               st.metric("Worst Day ROI", f"{worst_day_roi:.2f}%")
                           with col3:
                               volatility = daily_df['ROI (%)'].std()
                               st.metric("ROI Volatility", f"{volatility:.2f}%")
                           with col4:
